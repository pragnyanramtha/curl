...
| `--cookie`, `--cookie-jar` | partial | `TransferConfig::cookie`, `TransferConfig::cookie_files`, `TransferConfig::cookie_jar` | `empty_cookie_input_activates_cookie_engine`, `cookie_header_appends_repeated_literals`, `cookie_file_sends_netscape_cookie`, `cookie_file_and_literal_cookie_are_combined`, `cookie_jar_saves_response_cookies`, `cookie_jar_creates_header_only_file_without_cookies`, `cookie_jar_sends_cookie_on_later_url_in_group` | Basic direct Cookie headers, repeated literal joining, engine activation, Netscape/Set-Cookie file import, HTTP cookie engine, file-plus-literal sends, and Netscape jar output only; PSL/supercookie rejection, exact expiry/order/domain/path/sorting parity, redirect-cookie export, and full corpus mapping remain incomplete |
